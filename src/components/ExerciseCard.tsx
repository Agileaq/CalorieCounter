  ...
  return (
    <div className="card">
      ...
      {day.exercise.map(e => (
        <div key={e.id} className="row spread" style={{ padding: '6px 0' }}>
          ...
          <div className="row" style={{ gap: 8 }}><span>{e.caloriesBurned}</span>
            <button className="icon-btn" style={{ width: 28, height: 28, fontSize: 14 }} aria-label={t('common.deleteEntry', { name: e.name })} onClick={() => deleteExercise(e.id)}>✕</button></div>
        </div>
      ))}
      <div className="row" style={{ gap: 8, marginTop: 8 }}>
        ...
        <NumberInput testId="exercise-cals" integer value={exCals} onChange={setExCals} clearOnFocus style={{ width: 80 }} />
      </div>
