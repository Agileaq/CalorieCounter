...
import { DEFAULT_MACRO_RANGES } from './storage'
...
const SET = { dailyBudget: 2000, macroTargets: { carbs: 1, protein: 1, fat: 1, fiber: 1 }, macroRanges: DEFAULT_MACRO_RANGES, language: 'en' as const, goalWeightKg: null, adviceCutWeightKg: 0, adviceBulkWeightKg: 0 }
...
describe('importExport', () => {
  ...
  it('backup round-trips', () => {
    const data = { days: {}, myFoods: [f('Rice')], settings: { dailyBudget: 2012, macroTargets: { carbs: 1, protein: 1, fat: 1, fiber: 1 }, macroRanges: DEFAULT_MACRO_RANGES, language: 'en' as const, goalWeightKg: null, adviceCutWeightKg: 0, adviceBulkWeightKg: 0 } }
  ...
  it('backup round-trips food overrides', () => {
    const data = {
      ...
      settings: { dailyBudget: 2000, macroTargets: { carbs: 1, protein: 1, fat: 1, fiber: 1 }, macroRanges: DEFAULT_MACRO_RANGES, language: 'en' as const, goalWeightKg: null, adviceCutWeightKg: 0, adviceBulkWeightKg: 0 },
    }
    const parsed = parseBackup(exportBackup(data))
  ...
  it('mergeBackup takes incoming settings', () => {
    const ex = backup({ settings: { dailyBudget: 1000, macroTargets: { carbs: 1, protein: 1, fat: 1, fiber: 1 }, macroRanges: DEFAULT_MACRO_RANGES, language: 'en', goalWeightKg: null, adviceCutWeightKg: 0, adviceBulkWeightKg: 0 } })
    const inc = backup({ settings: { dailyBudget: 2500, macroTargets: { carbs: 9, protein: 8, fat: 7, fiber: 6 }, macroRanges: DEFAULT_MACRO_RANGES, language: 'zh' as const, goalWeightKg: null, adviceCutWeightKg: 0, adviceBulkWeightKg: 0 } })
    const merged = mergeBackup(ex, inc)
